...
# 4. Station Mapping & Prediction Logic
...
def get_next_expected(status, current_station, current_miles, avg_mph, mode):
    ...
    # Calculate distance to that next specific station
    next_stat_miles = m_map[next_name]
    dist_to_next = next_stat_miles - (current_miles % loop_size)
    if dist_to_next <= 0:
...
def get_status(row, mode):
    ...
    try:
        ...
        time_str, total_sec, avg_mph = "---", 999999, 0
